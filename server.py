...
DEFAULT_CONFIG = {
    "port": 9527,
    "save_paths": [
        os.path.join(HOME, "Desktop", "FX2md", "MD")
    ],
    # X/Twitter 书签按钮悬停菜单：命名保存路径（只在用户点击菜单项时使用）
    "custom_save_paths": [],
    "filename_format": "{summary}_{date}_{author}",
    "max_filename_length": 60,
    "video_save_path": os.path.join(HOME, "Desktop", "FX2md", "Videos"),
...
logger = logging.getLogger("fx2md")
...
"""
    ...
    vid_map = {}
    save_dir = cfg.get("video_save_path", os.path.join(HOME, "Desktop", "FX2md", "Videos"))
...
class FX2mdHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/ping":
            # 心跳检测
            self._respond(200, {"status": "ok", "version": "1.2.1"})
