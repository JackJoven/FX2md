...
function captureLinuxDoPostElement(post) {
    ...
    console.log("[fx2md] LINUX DO 帖子：", { url: data.url, author: data.author, title: data.article_title });
...
async function scrollAndCollectFeishuBlocks() {
    ...
    if (!container) {
        console.log("[fx2md] 未找到 .bear-web-x-container，跳过滚动收集");
        return null;
    }
    ...
    function getFeishuBlockTextScore(block) {
    ...
    function getFeishuTableScore(block) {
        const cells = Array.from(block?.querySelectorAll?.(".block[data-block-type='table_cell']") || []);
        const nonEmptyCells = cells.filter((cell) => getFeishuBlockTextScore(cell) > 0).length;
        return nonEmptyCells * 1000 + cells.length * 10 + getFeishuBlockTextScore(block);
    }

    function shouldReplaceCollectedBlock(existing, next) {
        const type = next?.getAttribute?.("data-block-type") || "";
        if (!existing) return true;
        if (type !== "table") return false;
        return getFeishuTableScore(next) > getFeishuTableScore(existing);
    }
    ...
    function collect() {
        ...
        for (const block of blocks) {
            ...
            if (shouldReplaceCollectedBlock(existing, clone)) {
                seen.set(id, clone);
            }
        }
    }
    ...
    console.log(`[fx2md] 飞书滚动收集完成：${seen.size} unique blocks`);
...
function captureFeishuDocument() {
    ...
    scrollAndCollectFeishuBlocks().then((collectedBlocks) => {
        ...
        if (!articleContent || articleContent.length < 50) {
            ...
            console.log("[fx2md] Feishu 文档（直接提取）：", { url: data.url, title: data.article_title });
        ...
        console.log("[fx2md] Feishu 文档（滚动收集 " + collectedBlocks.length + " blocks）：", { url: data.url, title });
        showToast("正在保存飞书文档…", "loading", null);
        sendToBackground(data);
    });
}
...
function captureWechatArticle() {
    ...
    console.log("[fx2md] WeChat 文章：", { url: data.url, author: data.author, title: data.article_title });
...
// X/Twitter 页面内复制正文按钮
// ─────────────────────────────────────────────
const X_INLINE_COPY_BUTTON_CLASS = "__fx2md_x_inline_copy_button";
const X_INLINE_TRANSLATE_BUTTON_CLASS = "__fx2md_x_inline_translate_button";
const X_INLINE_TRANSLATION_BLOCK_CLASS = "__fx2md_x_inline_translation_block";
const X_INLINE_ACTIONS_CONTAINER_CLASS = "__fx2md_x_inline_actions_container";
const X_INLINE_TRANSLATION_STATUS_CLASS = "__fx2md_x_inline_translation_status";
...
async function resolveContentForCopy(article, triggerButton) {
    ...
    if (payload.note_article_url) {
        try {
            ...
            console.warn("[fx2md] 后台提取 X Article 正文失败，回退当前 DOM：", error);
        }
    }
...
function clearTranslationMark(el) {
    if (!el) return;
    delete el.__fx2md_translation_override;
    el.removeAttribute?.("data-fx2md-translated");
}

function clearNativeTwitterTranslationOverride(scope = document) {
    const target = getTranslationTarget(scope);
    if (target?.textEl?.__fx2md_translation_override?.source === "twitter_native") {
...
function findMainTweetTextElement(article, options = {}) {
    ...
    for (const el of ctx.querySelectorAll('[data-testid="tweetText"]')) {
        ...
        if (!options.includeQuote && el.closest('[data-fx2md-quote-container="1"]')) continue;
        if (el.closest(`.${X_INLINE_TRANSLATION_BLOCK_CLASS}`)) continue;
        return el;
    }
    return null;
...
function findQuoteContainerFromLabel(labelEl, root) {
    ...
    while (current && current !== root && current !== document.body) {
        ...
        if (rect && rect.width > 220 && rect.height > 40 && hasQuoteContent && !current.matches?.('article[data-testid="tweet"]')) {
            current.setAttribute?.("data-fx2md-quote-container", "1");
            return current;
        }
...
function findQuoteTweetContainer(scope = document) {
    ...
    if (simple) {
        simple.setAttribute?.("data-fx2md-quote-container", "1");
        return simple;
    }
...
function markElementTranslated(el, override) {
    ...
    el.__fx2md_translation_override = override;
    el.setAttribute?.("data-fx2md-translated", "1");
}

function getElementTranslationOverride(el) {
    let current = el;
    while (current && current !== document.documentElement) {
        if (current.__fx2md_translation_override) return current.__fx2md_translation_override;
        current = current.parentElement;
    }
    return null;
}
...
function findDescendantTranslationOverride(scope) {
    const translated = scope?.querySelector?.('[data-fx2md-translated="1"]');
    return translated?.__fx2md_translation_override || null;
}

function restoreTranslatedElement(el) {
    if (!el || el.__fx2md_original_html === undefined) return false;
    el.innerHTML = el.__fx2md_original_html;
    delete el.__fx2md_original_html;
    delete el.__fx2md_translation_override;
    el.removeAttribute?.("data-fx2md-translated");
    return true;
}
...
function replaceElementTextWithTranslation(el, translatedText, override) {
    ...
    if (el.__fx2md_original_html === undefined) {
        el.__fx2md_original_html = el.innerHTML;
    }
...
function showOriginalTranslationTargets(scope = document) {
    ...
    if (target.kind === "article") {
        ...
        if (target.bodyEl) {
            delete target.bodyEl.__fx2md_translation_override;
            target.bodyEl.removeAttribute?.("data-fx2md-translated");
        }
        return restored;
    }
    ...
    if (target.kind === "article_card") {
        ...
        if (target.cardEl) {
            delete target.cardEl.__fx2md_translation_override;
            target.cardEl.removeAttribute?.("data-fx2md-translated");
        }
        return restored;
    }
    ...
    if (target.kind === "quote_tweet") {
        ...
        if (target.quoteEl) {
            delete target.quoteEl.__fx2md_translation_override;
            target.quoteEl.removeAttribute?.("data-fx2md-translated");
        }
        return restored;
    }
...
function targetHasVisibleTranslation(target) {
    if (!target) return false;
    if (target.kind === "article") {
        if (target.titleEl?.__fx2md_translation_override) return true;
        return getArticleTranslatableTextBlocks(target.bodyEl).some((block) => !!block.__fx2md_translation_override);
    }
    if (target.kind === "article_card") {
        return !!target.titleEl?.__fx2md_translation_override || !!target.bodyEl?.__fx2md_translation_override;
    }
    if (target.kind === "quote_tweet") {
        return !!target.textEl?.__fx2md_translation_override;
    }
    return !!target.textEl?.__fx2md_translation_override;
}
...
function createNativeLikeTranslationBlock(target) {
    ...
    block.innerHTML = `<div data-fx2md-role="translated-status" class="${X_INLINE_TRANSLATION_STATUS_CLASS}" style="display:none;color:rgb(83,100,113);font-size:13px;line-height:1.35;"></div>`;
...
function setInlineTranslationStatus(scope, message) {
    ...
    const statusEl = block?.querySelector('[data-fx2md-role="translated-status"]');
...
async function translateArticleCardInPlace(target, options = {}) {
    ...
    if (tweetId && !options.skipNativeTweetTranslation) {
        try {
            ...
            console.warn("[fx2md] Article 卡片原生翻译失败，回退文本翻译：", error);
        }
    }
...
async function translateTweetTextTargetInPlace(target, options = {}) {
    ...
    if (tweetId) {
        try {
            ...
            console.warn("[fx2md] 推文翻译失败，回退普通文本翻译：", error);
        }
    }
...
function getTranslationOverrideForSave(scope = document) {
    ...
    const override = block?.__fx2md_translation_override;
...
async function translateScopeInline(scope = document, options = {}) {
    ...
    if (tweetId) {
        try {
            ...
            console.warn("[fx2md] Grok 翻译失败，回退普通文本翻译：", error);
        }
    }
    if (!translatedText) {
        const result = await requestBackgroundTextTranslation({
...
function drainAutoTranslateQueue() {
    while (xAutoTranslateActiveCount < X_AUTO_TRANSLATE_MAX_CONCURRENCY && xAutoTranslateQueue.length) {
        ...
        translateScopeInline(item.scope, { force: false, auto: true })
            ...
            .catch((error) => {
                console.warn("[fx2md] 自动翻译失败：", error);
            })
            .finally(() => {
...
function buildTwitterInlineTranslateButton(referenceButton) {
    ...
    btn.addEventListener("pointerdown", (event) => {
        clearLongPressTimer();
        btn.__fx2md_long_press_fired = false;
        longPressTimer = setTimeout(() => {
            btn.__fx2md_long_press_fired = true;
            try {
    ...
    btn.addEventListener("click", async (event) => {
        ...
        event.stopPropagation();
        clearLongPressTimer();
        if (btn.__fx2md_long_press_fired) {
            btn.__fx2md_long_press_fired = false;
            return;
        }
        ...
        try {
            ...
            console.error("[fx2md] 翻译失败：", error);
...
function buildTwitterInlineCopyButton(referenceButton) {
    ...
    btn.addEventListener("click", async (event) => {
        ...
        try {
            ...
            console.error("[fx2md] 复制正文失败：", error);
...
const X_CUSTOM_SAVE_MENU_ID = "__fx2md_x_custom_save_menu";
    ...
    entries.forEach((entry, index) => {
        ...
        item.addEventListener("click", (event) => {
            ...
            if (!isBookmarkButtonAlreadySaved(btn)) {
                btn.__fx2md_skip_next_default_bookmark_save = true;
                btn.click();
                setTimeout(() => {
                    if (btn.__fx2md_skip_next_default_bookmark_save) {
                        btn.__fx2md_skip_next_default_bookmark_save = false;
                    }
                }, 0);
            }
            setTimeout(() => {
...
function attachBookmarkListener(btn) {
    if (btn.__fx2md_bound) return;
    btn.__fx2md_bound = true;
...
function captureAndSend(btn, options = {}) {
    ...
    if (noteArticleUrl) {
        console.log("[fx2md] 检测到 Note 推文，文章链接：", noteArticleUrl);
        ...
        try {
            ...
            if (inlineArticle && inlineArticle.article_content && inlineArticle.article_content.trim().length > 50) {
                console.log("[fx2md] 当前页面已有内嵌文章内容，直接保存");
    ...
    console.log("[fx2md] 普通推文：", { handle, url: tweetUrl, text: text.slice(0, 40), hasQuote: !!quote_tweet });
    sendToBackground(attachCustomSavePath(withVisibleTranslationOverride({
        author,
        handle,
        text,
...
function sendToBackground(data) {
    chrome.runtime.sendMessage({ action: "save_tweet", data }, (resp) => {
        if (chrome.runtime.lastError) {
            console.error("[fx2md] 扩展通信失败：", chrome.runtime.lastError);
...
function getToast() {
    let t = document.getElementById("__fx2md_toast");
    if (!t) {
...
console.log("[fx2md] 内容脚本已加载 v1.4");
