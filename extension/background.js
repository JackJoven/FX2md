/**
 * background.js - FX2md Service Worker v1.4
 *
 * 获取完整推文的 3 层策略（依次降级）：
...
async function discoverGraphQLOperationIdsFromPage(pageUrl) {
    ...
    try {
        ...
        console.warn("[fx2md] 自动探测 GraphQL operation id 失败：", error);
    }
    ...
    return new Promise((resolve) => {
        ...
        const timeout = setTimeout(() => {
            if (!resolved) {
                ...
                console.warn("[fx2md] Note tab 超时");
                resolve(null);
            }
        }, 15000);
                ...
                const poll = () => {
                    ...
                    chrome.scripting.executeScript(
                        ...
                        (results) => {
                            ...
                            clearTimeout(timeout);
                            chrome.tabs.remove(tabId).catch(() => { });
                            console.log(`[fx2md] Note 内容提取成功：title="${result.title.slice(0, 30)}" 长度=${result.content.length}`);
                            resolve(result);
                        }
                    );
                };
...
async function fetchViaGraphQL(tweetId, options = {}) {
    try {
        ...
        const csrfToken = await getCookieValue("ct0");
        if (!csrfToken) {
            console.warn("[fx2md] 未找到 ct0 cookie，跳过 GraphQL");
            return null;
        }
        ...
        for (const plan of plans) {
            ...
            if (!resp.ok) {
                console.warn(`[fx2md] ${plan.operationName}(${plan.operationId}) 返回 ${resp.status}`);
                continue;
            }
            ...
            if (!mainTweet) {
                console.warn(`[fx2md] ${plan.operationName}(${plan.operationId}) 未找到目标推文`);
                continue;
            }
        ...
        console.warn("[fx2md] 所有 GraphQL 候选请求均失败");
        return null;

    } catch (err) {
        console.error("[fx2md] GraphQL API 异常：", err);
        return null;
    }
}
...
async function fetchViaOEmbed(tweetUrl) {
    try {
        ...
        console.error("[fx2md] oEmbed API 异常：", err);
        return null;
    }
}
...
async function fetchFullTweetData(tweetData) {
    ...
    console.log("[fx2md] 开始获取完整推文：", tweetId);
    ...
    if (!apiResult || !apiResult.text) {
        console.log("[fx2md] GraphQL 失败，尝试 oEmbed");
        apiResult = await fetchViaOEmbed(tweetData.url);
    }

    if (!apiResult) {
        console.log("[fx2md] 所有 API 均失败，使用 DOM 原始数据");
        return tweetData;
    }

    console.log(`[fx2md] API 获取成功：text=${apiResult.text.slice(0, 50)} images=${apiResult.images.length}`);
...
function applyCustomSavePathSelection(data, config) {
    const selection = data?.fx2md_custom_save_path;
    ...
    delete nextData.fx2md_custom_save_path;
    return nextData;
}
        ...
        (async () => {
            try {
                ...
                if (data.type === "note" && data.note_article_url) {
                    ...
                    console.log("[fx2md] 处理 Note（后台 tab 方案）：", data.note_article_url);
                    ...
                    if (noteResult && noteResult.content) {
                        ...
                        // 获取失败：降级为推文摘要 + 文章链接
                        console.warn("[fx2md] Note 内容获取失败，降级为摘要");
                        data = {
                            ...data,
                            type: "tweet",
                    ...
                    if (articleMatch) {
                        ...
                        console.log("[fx2md] 在推文提取文本中发现长文(Note)链接，切换提取模式：", articleUrl);
                ...
                if (contentToFix.includes("[[VIDEO_HOLDER_")) {
                    console.log("[fx2md] 检测到长文包含未解析的视频占位符，尝试通过 GraphQL 兜底获取");
                ...
                } catch (e) { console.warn("[fx2md] 获取配置失败，使用默认视频设置", e); }
                ...
                if (enableVideoDownload && allVideos.length > 0 && !data.video_confirmed) {
                    ...
                    if (maxDurationMin > durationThresholdMin) {
                        console.log(`[fx2md] 发现超长视频 (${maxDurationMin.toFixed(1)} > ${durationThresholdMin} min)，要求前台确认`);
                        sendResponse({
    ...
    if (message.action === "translate_tweet") {
        (async () => {
            try {
                ...
                console.error("[fx2md] 获取推文翻译失败：", err);
    ...
    if (message.action === "translate_text") {
        (async () => {
            try {
                ...
                console.error("[fx2md] 文本翻译失败：", err);
                sendResponse({ success: false, error: err.message || String(err) });
            }
        })();
        return true;
    ...
    if (message.action === "copy_content_text") {
        (async () => {
            try {
                ...
                console.error("[fx2md] 复制正文提取失败：", err);
                sendResponse({ success: false, error: err.message || String(err) });
            }
        })();
        return true;
