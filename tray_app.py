...
def get_app_dir():
    ...
    Mac: ~/Library/Application Support/FX2md
    Windows: %APPDATA%/FX2md
    这样升级 app 不丢配置。"""
    if sys.platform == "darwin":
        d = os.path.join(os.path.expanduser("~"), "Library", "Application Support", "FX2md")
    elif sys.platform == "win32":
        d = os.path.join(os.environ.get("APPDATA", os.path.expanduser("~")), "FX2md")
    else:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(APP_DIR, "fx2md.log"), encoding="utf-8"),
        logging.StreamHandler(sys.stdout),
    ]
)
logger = logging.getLogger("fx2md_tray")
...
def start_server_thread():
    ...
    from server import FX2mdHandler, load_config

    cfg = load_config()
    port = cfg.get("port", 9527)
    server = HTTPServer(("127.0.0.1", port), FX2mdHandler)
    _server_ref = server

    logger.info(f"🚀 fx2md 服务已启动，监听 http://127.0.0.1:{port}")
...
def run_tray():
    ...
    def on_open_log(icon, item):
        log_file = os.path.join(APP_DIR, "fx2md.log")
        if sys.platform == "darwin":
    ...
    menu = pystray.Menu(
        pystray.MenuItem("FX2md 服务运行中", None, enabled=False),
        ...
        pystray.Menu.SEPARATOR,
        pystray.MenuItem("❌ 退出 FX2md", on_quit),
    )

    icon = pystray.Icon("fx2md", icon_image, "FX2md", menu)
...
def run_cli_mode():
    ...
    logger.info("🚀 FX2md 服务已启动，按 Ctrl+C 停止")
